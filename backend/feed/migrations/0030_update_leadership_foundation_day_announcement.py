from django.db import migrations
from django.utils import timezone


ANNOUNCEMENT_TITLE = "🎉 Acuite Foundation Day Celebration"
ANNOUNCEMENT_BODY = (
    "On Acuite's 21st Foundation Day, let's come together to make this milestone truly "
    "special and memorable. Let's celebrate our journey, our people, and the values "
    "that continue to shape Acuite's success."
)
ANNOUNCEMENT_DATE = "5 September 2026"
ANNOUNCEMENT_WHERE = "All Acuite Offices"
ANNOUNCEMENT_HOST = "MD & CEO, along with the Leadership Team"
ANNOUNCEMENT_FORMAT = "21 Years of Trust, Innovation & Excellence"


def update_leadership_foundation_day_announcement(apps, schema_editor):
    Post = apps.get_model("feed", "Post")
    User = apps.get_model("accounts", "User")

    existing_post = None
    for post in Post.objects.filter(module="bulletin", topic="announcements").order_by("-created_at"):
        metadata = dict(post.metadata or {})
        tag = str(metadata.get("home_announcement_tag", "")).strip().lower()
        if tag == "leadership":
            existing_post = post
            break

    author = (
        User.objects.filter(
            is_active=True,
            employment_status="active",
            access_level="admin",
        )
        .order_by("id")
        .first()
    ) or User.objects.filter(is_active=True).order_by("id").first()

    if not existing_post and not author:
        return

    metadata = {
        "bulletin_category": "announcements",
        "bulletin_channel": "announcements",
        "home_announcement_tag": "leadership",
        "home_announcement_type": "other",
        "home_announcement_town_hall": None,
        "bulletin_meta_lines": [
            f"When: {ANNOUNCEMENT_DATE}",
            f"Where: {ANNOUNCEMENT_WHERE}",
            f"Hosted by: {ANNOUNCEMENT_HOST}",
        ],
        "home_announcement_display": {
            "typeLabel": "Leadership",
            "formatLabel": ANNOUNCEMENT_FORMAT,
            "dateLabel": ANNOUNCEMENT_DATE,
            "timeLabel": "Foundation Day Celebration",
            "venueLabel": ANNOUNCEMENT_WHERE,
            "hostLabel": ANNOUNCEMENT_HOST,
            "audienceLabel": "Open to all employees",
            "countdownLabel": ANNOUNCEMENT_FORMAT,
            "summary": ANNOUNCEMENT_BODY,
            "details": [],
        },
        "post_as_company": True,
        "company_author_name": "Acuite Ratings & Research",
        "company_author_title": "Official company post",
        "company_author_initials": "AR",
    }

    if existing_post:
        existing_post.title = ANNOUNCEMENT_TITLE
        existing_post.body = ANNOUNCEMENT_BODY
        existing_post.kind = "announcement"
        existing_post.module = "bulletin"
        existing_post.topic = "announcements"
        existing_post.visibility = "company"
        existing_post.moderation_status = "published"
        existing_post.allow_comments = True
        existing_post.metadata = metadata
        if not existing_post.published_at:
            existing_post.published_at = timezone.now()
        existing_post.save()
        return

    Post.objects.create(
        author=author,
        title=ANNOUNCEMENT_TITLE,
        body=ANNOUNCEMENT_BODY,
        kind="announcement",
        module="bulletin",
        topic="announcements",
        visibility="company",
        moderation_status="published",
        allow_comments=True,
        published_at=timezone.now(),
        metadata=metadata,
    )


class Migration(migrations.Migration):
    dependencies = [
        ("feed", "0029_add_leadership_townhall_join_details"),
    ]

    operations = [
        migrations.RunPython(
            update_leadership_foundation_day_announcement,
            migrations.RunPython.noop,
        ),
    ]
